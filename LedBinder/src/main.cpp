...
// OUT: clamp/closed
static const uint8_t SERVO1_FULL_OPEN = 82;
static const uint8_t SERVO2_FULL_OPEN = 82;
...
static uint8_t mode5_step = 0;        // 0..3
...
void mode5StepAdvance()
{
	// MODE 5 sequence:
	// step 0: close
	// step 1: twist N times
	// step 2: full open (drop)
	// step 3: partial open (ready)
	switch (mode5_step) {
		case 0:
			moveServosOut();
			delay(CLAMP_HOLD_MS);
...
void loop()
{
	...
	mode5StepAdvance();
#else
	#error "Invalid MODE. Use MODE 1, 2, 3, 4, or 5."
#endif
}
